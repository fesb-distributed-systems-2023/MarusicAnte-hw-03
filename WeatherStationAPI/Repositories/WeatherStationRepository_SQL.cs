...
namespace WeatherStationAPI.Repositories
{
    public class WeatherStationRepository_SQL : IWeatherStationRepository
    {
        ...
        public bool CreateNewWeatherStation(WeatherStation weatherStation)
        {
            ...
            command.CommandText =
            @"
                INSERT INTO WeatherStations (Name, Location, Temperature, Humidity, WindSpeed)
                VALUES ($name, $location, $temperature, $humidity, $windSpeed)";
        ...
        public IEnumerable<WeatherStation> GetAllWeatherStations()
        {
            ...
            command.CommandText =
            @"
     SELECT ID, Name, Location, Temperature, Humidity, WindSpeed FROM WeatherStations";
            ...
            while (reader.Read())
            {
                var row = new WeatherStation
                {
                    ...
                    Humidity = reader.GetFloat(4), 
                    WindSpeed = reader.GetFloat(5) 
                };
        ...
        public WeatherStation? GetWeatherStation(int id)
        {
            ...
            var command = connection.CreateCommand();
            command.CommandText =
                @"SELECT ID, Name, Temperature, Humidity, WindSpeed FROM WeatherStations
                         WHERE ID == $id";
